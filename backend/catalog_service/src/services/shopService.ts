...
function parsePagination(query: ListPublicShopsQuery): { page: number; limit: number } {
...
function parseOptionalPublicShopStatus(value: unknown): ShopStatus | undefined {
    ...
    const allowed: ShopStatus[] = [ShopStatus.ACTIVE, ShopStatus.INACTIVE];
...
export async function listPublicShops(query: ListPublicShopsQuery) {
    const { page, limit } = parsePagination(query);
    const q = typeof query.q === "string" && query.q.trim() ? query.q.trim() : undefined;
    const status = parseOptionalPublicShopStatus(query.status) ?? ShopStatus.ACTIVE;
