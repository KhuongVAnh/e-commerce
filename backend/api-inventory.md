...
| `POST /api/uploads/images` | Upload ảnh lên Cloudinary | `multipart/form-data`<br>Field file: `image` | `url: string`<br>`publicId: string`<br>`width: number`<br>`height: number`<br>`format: string`<br>`bytes: number` |
 ...
 | `GET /api/catalog/seller/products?page&limit&keyword&q&categoryId&status&minPrice&maxPrice&sortBy` | Lấy danh sách sản phẩm thuộc shop của seller đang đăng nhập, có lọc theo category, trạng thái, khoảng giá và sắp xếp; bao gồm `ACTIVE`, `INACTIVE`, `OUT_OF_STOCK`; không trả sản phẩm đã xóa mềm | Không có body | `products: [ { id, shopId, categoryId, name, slug, description, price, stockQuantity, thumbnailUrl, status, deletedAt, createdAt, updatedAt, shop, category, images } ]`<br>`pagination: { page, limit, total, totalPages }`<br>`filters: { keyword, categoryId, status, minPrice, maxPrice, sortBy }`<br>`sortBy: latest, oldest, price_asc, price_desc, stock_asc, stock_desc, name_asc, name_desc` |
 | `GET /api/catalog/seller/products/:productId` | Lấy chi tiết sản phẩm thuộc shop của seller đang đăng nhập, bao gồm sản phẩm hết hàng hoặc tạm ẩn | Không có body | `product: { id, shopId, categoryId, name, slug, description, price, stockQuantity, thumbnailUrl, status, deletedAt, createdAt, updatedAt, shop, category, images }` |
