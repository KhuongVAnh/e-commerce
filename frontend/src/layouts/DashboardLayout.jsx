...
const DashboardLayout = ({ roleTitle }) => {
  ...
  return (
    <div className="flex h-screen bg-slate-50 font-body overflow-hidden">
      ...
      <aside
        className={`fixed md:static inset-y-0 left-0 z-40 w-64 bg-white border-r border-slate-100 flex flex-col shadow-2xl md:shadow-sm transform transition-transform duration-300 ${
      ...
      >
        ...
        <nav className="flex-1 p-4 space-y-2 text-sm font-bold overflow-y-auto">
          {menuItems.map((item, index) => (
            <NavLink
              key={index}
              to={item.path}
              end={item.path === '/seller' || item.path === '/admin'}
              onClick={() => setIsMobileMenuOpen(false)}
              className={({ isActive }) =>
          ...
          {/* ĐÂY LÀ LỐI THOÁT CHO ADMIN: Hiển thị nếu Admin đang vào xem trang Seller */}
        ...
        <div className="p-6 border-t border-slate-50">
          <div className="flex items-center justify-between cursor-pointer">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 bg-slate-800 text-white rounded-full flex items-center justify-center text-xs font-bold uppercase">
                {profileAvatar}
              </div>
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden w-full">
        <header className="h-20 bg-white/80 backdrop-blur-md border-b border-slate-100 flex items-center justify-end px-4 md:px-8 z-10 shrink-0">
          <div className="flex items-center gap-5">
            ...
            <div className="w-8 h-8 rounded-full bg-slate-800 flex items-center justify-center text-white text-xs font-bold uppercase cursor-pointer">
              {profileAvatar}
            </div>
          </div>
        </header>
