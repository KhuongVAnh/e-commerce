...
const CustomerLayout = () => {
  ...
  const handleCopyLink = (e) => {
    ...
    navigator.clipboard.writeText(window.location.origin)
      .then(() => toast.success(t('Đã sao chép liên kết trang web!')))
  ...
  return (
    <div className="bg-surface text-on-surface min-h-screen font-body flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 max-w-screen-2xl mx-auto">
          ...
          <div className="flex items-center gap-6">
            <nav className="hidden md:flex items-center gap-8">
              <Link to="/" className={getNavClass(isHomeActive)}>{t('Trang chủ')}</Link>
              <Link to="/products" className={getNavClass(isProductsActive)}>{t('Sản phẩm')}</Link>
              <Link to="/shop" className={getNavClass(isShopActive)}>{t('Cửa hàng')}</Link>
              <Link to="/categories" className={getNavClass(isCategoriesActive)}>{t('Danh mục')}</Link>
            </nav>
            
            <div className="flex items-center gap-4">
              <button 
                onClick={toggleLanguage} 
                className="w-9 h-9 flex items-center justify-center rounded-full bg-indigo-50 text-[#2b3896] font-bold text-xs hover:bg-indigo-100 transition-colors"
      ...
      <footer className="bg-slate-900 text-slate-300 py-12 md:py-16 border-t border-slate-800 font-body mt-auto">
        <div className="max-w-screen-2xl mx-auto px-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
          <div className="space-y-4">
            ...
            <p className="text-sm text-slate-400 leading-relaxed pr-4">{t('Khám phá bộ sưu tập đồ công nghệ và thời trang cao cấp được tuyển chọn kỹ lưỡng dành riêng cho bạn.')}</p>
          </div>
          <div>
            <h3 className="text-white font-semibold mb-6 uppercase tracking-wider text-sm">{t('Chăm sóc khách hàng')}</h3>
            <ul className="space-y-3 text-sm">
              <li><Link to="/help#faq" className="hover:text-white transition-colors">{t('Trung tâm trợ giúp')}</Link></li>
              <li><Link to="/help#shipping" className="hover:text-white transition-colors">{t('Chính sách vận chuyển')}</Link></li>
              <li><Link to="/help#returns" className="hover:text-white transition-colors">{t('Trả hàng & Hoàn tiền')}</Link></li>
              <li><Link to="/help#contact" className="hover:text-white transition-colors">{t('Liên hệ với chúng tôi')}</Link></li>
            </ul>
          </div>
          <div>
            <h3 className="text-white font-semibold mb-6 uppercase tracking-wider text-sm">{t('Về thương hiệu')}</h3>
            <ul className="space-y-3 text-sm">
              <li><Link to="/about#intro" className="hover:text-white transition-colors">{t('Giới thiệu về chúng tôi')}</Link></li>
              <li><Link to="/about#careers" className="hover:text-white transition-colors">{t('Tuyển dụng')}</Link></li>
              <li><Link to="/about#terms" className="hover:text-white transition-colors">{t('Điều khoản dịch vụ')}</Link></li>
              <li><Link to="/about#privacy" className="hover:text-white transition-colors">{t('Chính sách bảo mật')}</Link></li>
            </ul>
          </div>
          <div>
            <h3 className="text-white font-semibold mb-6 uppercase tracking-wider text-sm">{t('Thanh toán an toàn')}</h3>
            ...
            <h3 className="text-white font-semibold mb-4 uppercase tracking-wider text-sm">{t('Kết nối')}</h3>
            <div className="flex gap-4">
              <button onClick={handleCopyLink} className="w-10 h-10 rounded-full bg-slate-800 flex items-center justify-center hover:bg-[#2b3896] hover:text-white transition-colors">
                 <span className="material-symbols-outlined text-xl">share</span>
              </button>
              <Link to="/help#contact" className="w-10 h-10 rounded-full bg-slate-800 flex items-center justify-center hover:bg-[#2b3896] hover:text-white transition-colors">
        ...
        <div className="max-w-screen-2xl mx-auto px-6 mt-12 pt-8 border-t border-slate-800 text-sm text-center text-slate-500 flex flex-col md:flex-row justify-between items-center gap-4">
          <p>&copy; {t('2026 E-Commerce. Mọi quyền được bảo lưu.')}</p>
          <p>{t('Dự án E-Commerce - Nhóm 31')}</p>
        </div>
      </footer>
      ...
      <nav className="fixed bottom-0 left-0 w-full z-50 flex justify-around items-center px-2 pt-3 pb-8 md:hidden bg-white/90 backdrop-blur-2xl shadow-[0_-4px_20px_rgba(43,56,150,0.08)] border-t border-slate-100">
        <Link to="/" className={`flex flex-col items-center justify-center rounded-xl px-3 py-1.5 transition-all ${isHomeActive ? 'bg-[#2b3896]/10 text-[#2b3896]' : 'text-slate-500 hover:text-[#2b3896]'}`}>
          <span className="material-symbols-outlined" style={{ fontVariationSettings: isHomeActive ? "'FILL' 1" : "'FILL' 0" }}>home</span>
          <span className="text-[11px] font-medium mt-1">{t('Trang chủ')}</span>
        </Link>
        <Link to="/shop" className={`flex flex-col items-center justify-center rounded-xl px-3 py-1.5 transition-all ${isShopActive ? 'bg-[#2b3896]/10 text-[#2b3896]' : 'text-slate-500 hover:text-[#2b3896]'}`}>
          <span className="material-symbols-outlined" style={{ fontVariationSettings: isShopActive ? "'FILL' 1" : "'FILL' 0" }}>storefront</span>
          <span className="text-[11px] font-medium mt-1">{t('Cửa hàng')}</span>
        </Link>
        <Link to="/products" className={`flex flex-col items-center justify-center rounded-xl px-3 py-1.5 transition-all ${isProductsActive ? 'bg-[#2b3896]/10 text-[#2b3896]' : 'text-slate-500 hover:text-[#2b3896]'}`}>
          <span className="material-symbols-outlined" style={{ fontVariationSettings: isProductsActive ? "'FILL' 1" : "'FILL' 0" }}>box</span>
          <span className="text-[11px] font-medium mt-1">{t('Sản phẩm')}</span>
        </Link>
        <Link to="/categories" className={`flex flex-col items-center justify-center rounded-xl px-3 py-1.5 transition-all ${isCategoriesActive ? 'bg-[#2b3896]/10 text-[#2b3896]' : 'text-slate-500 hover:text-[#2b3896]'}`}>
          <span className="material-symbols-outlined" style={{ fontVariationSettings: isCategoriesActive ? "'FILL' 1" : "'FILL' 0" }}>category</span>
          <span className="text-[11px] font-medium mt-1">{t('Danh mục')}</span>
        </Link>
        <Link to="/profile" className={`flex flex-col items-center justify-center rounded-xl px-3 py-1.5 transition-all ${isProfileActive ? 'bg-[#2b3896]/10 text-[#2b3896]' : 'text-slate-500 hover:text-[#2b3896]'}`}>
          <span className="material-symbols-outlined" style={{ fontVariationSettings: isProfileActive ? "'FILL' 1" : "'FILL' 0" }}>person</span>
          <span className="text-[11px] font-medium mt-1">{t('Hồ sơ')}</span>
        </Link>
      </nav>
    </div>
  );
