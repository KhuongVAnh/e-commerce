...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.password !== formData.confirmPassword) {
      setErrorMsg(t("Mật khẩu xác nhận không khớp!"));
      return;
    }
    ...
    try {
      ...
      alert(t("Đăng ký thành công! Vui lòng đăng nhập."));
      navigate('/login');
      ...
      if (error && error.fieldErrors && error.fieldErrors.length > 0) {
        ...
        setErrorMsg(t("Đăng ký thất bại. Vui lòng thử lại."));
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="font-sans text-gray-900 min-h-screen flex flex-col relative">
      <main className="flex-grow flex flex-col items-center justify-center relative overflow-hidden px-4 py-12">
        ...
        <div className="mb-10 text-center z-10 w-full flex flex-col items-center pointer-events-none">
          ...
          <p className="text-gray-600 tracking-wide uppercase text-xs font-bold drop-shadow-sm bg-white/50 px-4 py-1 rounded-full backdrop-blur-sm mt-2">
            {t('Sàn thương mại điện tử dành cho bạn')}
          </p>
        </div>
        ...
        <section className="w-full max-w-md z-10">
          <div className="bg-white/90 backdrop-blur-xl shadow-[0px_12px_32px_rgba(43,56,150,0.15)] rounded-3xl p-8 md:p-10 border border-white/40">
            <header className="mb-8 text-center">
              <h2 className="font-extrabold text-2xl text-gray-900 leading-tight tracking-tight">{t('Tạo Tài Khoản')}</h2>
              <p className="text-gray-500 text-sm mt-2 font-medium">{t('Bắt đầu hành trình mua sắm & kinh doanh của bạn.')}</p>
            </header>
            ...
            <form className="space-y-6" onSubmit={handleSubmit}>
              ...
              <div className="space-y-3">
                <span className="text-xs font-bold uppercase tracking-wider text-gray-500 ml-1">{t('Bạn muốn đăng ký làm:')}</span>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-gray-500 ml-1 uppercase tracking-widest">{t('Họ và Tên')}</label>
                  <input 
                    name="fullName"
                    type="text" 
                    required
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-gray-500 ml-1 uppercase tracking-widest">{t('Địa chỉ Email')}</label>
                  <input 
                    name="email"
                    type="email" 
                    required
                    ...
                    onChange={handleChange}
                    placeholder={t("nguyenvana@example.com")} 
                    className="w-full bg-gray-50 border border-gray-200 rounded-xl px-5 py-4 text-gray-900 placeholder-gray-400 focus:ring-2 focus:border-[#2b3896] focus:ring-[#2b3896]/40 transition-all outline-none text-sm font-medium"
                  />
                </div>
                ...
                <div className="grid grid-cols-1 gap-4">
                  <div className="space-y-1.5">
                    <label className="text-xs font-bold text-gray-500 ml-1 uppercase tracking-widest">{t('Mật khẩu')}</label>
                  ...
                  <div className="space-y-1.5">
                    <label className="text-xs font-bold text-gray-500 ml-1 uppercase tracking-widest">{t('Xác nhận mật khẩu')}</label>
                    <div className="relative">
                      <input 
              ...
              <div className="pt-2">
                ...
                >
                  {loading ? t('Đang xử lý...') : t('Đăng Ký Tài Khoản')}
                </button>
              </div>
            </form>
            ...
            <div className="mt-8 text-center">
              <p className="text-gray-500 text-sm font-medium">
                ...
                <Link to="/login" className="text-[#2b3896] font-bold hover:underline transition-all ml-1">{t('Đăng Nhập')}</Link>
              </p>
            </div>
          </div>
          ...
          <div className="mt-8 px-6 text-center">
            <p className="text-gray-400 text-[11px] leading-relaxed uppercase tracking-widest font-semibold drop-shadow-sm bg-white/50 backdrop-blur-sm rounded-lg p-2 inline-block">
              {t('Bằng việc đăng ký, bạn đồng ý với')} <Link to="/terms" className="hover:text-[#2b3896] transition-colors underline">{t('Điều khoản')}</Link> & <Link to="/privacy" className="hover:text-[#2b3896] transition-colors underline">{t('Bảo mật')}</Link>
            </p>
          </div>
        </section>
      </main>
      ...
      <footer className="w-full py-6 text-center border-t border-gray-200 bg-white/50 backdrop-blur-sm relative z-10">
        <p className="text-xs text-gray-500 tracking-widest uppercase font-bold">© {t('2026 Đồ án E-Commerce. Nhóm 31.')}</p>
      </footer>
    </div>
  );
};
