  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (error?.error?.fieldErrors?.length > 0) {
        ...
        setErrorMsg(t("Đăng nhập thất bại. Vui lòng kiểm tra lại hệ thống."));
      }
    } finally {
      setLoading(false);
    }
            ...
            {step === 1 && (
              <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                <header className="mb-8 text-center">
                  <h2 className="font-extrabold text-2xl text-gray-900 leading-tight tracking-tight mb-2">{t('Bạn là ai?')}</h2>
                  <p className="text-sm text-gray-500 font-medium">{t('Vui lòng chọn tư cách đăng nhập')}</p>
                </header>
                ...
                <div className="space-y-4">
                  ...
                  >
                    ...
                    <div className="text-left">
                      <h3 className="font-bold text-gray-900 text-lg">{t('Khách hàng')}</h3>
                      <p className="text-xs text-gray-500 font-medium">{t('Mua sắm & Theo dõi đơn hàng')}</p>
                    </div>
                  </button>
                  ...
                  >
                    ...
                    <div className="text-left">
                      <h3 className="font-bold text-gray-900 text-lg">{t('Người bán hàng')}</h3>
                      <p className="text-xs text-gray-500 font-medium">{t('Quản lý gian hàng & Sản phẩm')}</p>
                    </div>
                  </button>
                  ...
                  >
                    ...
                    <div className="text-left">
                      <h3 className="font-bold text-gray-900 text-lg">{t('Quản trị viên')}</h3>
                      <p className="text-xs text-gray-500 font-medium">{t('Truy cập hệ thống quản lý')}</p>
                    </div>
                  </button>
                </div>
              </div>
              ...
              <div className="animate-in fade-in slide-in-from-right-8 duration-500">
                <header className="mb-6 relative flex items-center justify-center">
                  <button 
                    ...
                    title={t("Quay lại chọn vai trò")}
                  >
                    <ArrowLeft size={20} />
                  </button>
                  <div className="text-center">
                    <h2 className="font-extrabold text-2xl text-gray-900 leading-tight tracking-tight">{t('Đăng Nhập')}</h2>
                    <p className="text-xs font-bold text-[#2b3896] uppercase tracking-widest mt-1">
                      {t('Dành cho')} {role === 'SELLER' ? t('Người Bán') : role === 'ADMIN' ? t('Quản Trị Viên') : t('Khách Hàng')}
                    </p>
                  </div>
                </header>
                ...
                <form className="space-y-6" onSubmit={handleSubmit}>
                  ...
                  {(role === 'CUSTOMER' || role === 'SELLER') && (
                    <p className="text-center mt-6 text-sm text-gray-500 font-medium">
                      ...
                      <Link to="/register" className="text-[#2b3896] hover:underline font-bold transition-all ml-1">
                        {t('Đăng Ký')}
                      </Link>
                    </p>
                  )}
