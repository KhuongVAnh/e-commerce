...
import { orderService } from '../../services/orderService'; // Đảm bảo import đúng đường dẫn
...
export default function OrderDetail() {
    ...
    // State quản lý Popup thông báo lỗi
        ...
        const fetchDetail = async () => {
            try {
                ...
                if (responseData?.success && responseData?.data?.order) {
                    ...
                    setErrorPopup({ isOpen: true, message: 'Không thể tải dữ liệu đơn hàng. Cấu trúc dữ liệu không khớp.' });
                }
            } catch (error) {
                console.error("Lỗi lấy chi tiết đơn hàng:", error);
                setErrorPopup({ isOpen: true, message: error.response?.data?.message || 'Có lỗi xảy ra khi lấy thông tin đơn hàng.' });
            } finally {
                setLoading(false);
            }
        };
                ...
                {errorPopup.isOpen && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm px-4">
                        <div className="bg-white rounded-2xl p-8 max-w-sm w-full shadow-2xl transform transition-all text-center">
                            ...
                            <button onClick={closeErrorPopup} className="w-full bg-gray-100 text-gray-800 font-bold py-3 rounded-xl hover:bg-gray-200 transition-colors">
                                Đóng
                            </button>
                        </div>
                    </div>
    ...
    // Logic tính toán hiển thị thanh tiến trình (Stepper)
        ...
        <main className="pt-32 pb-24 px-6 md:px-12 max-w-screen-xl mx-auto font-['Inter'] bg-[#f9f9fc] min-h-screen">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
                ...
                <div className="flex gap-4">
                    <button className="bg-gray-200/50 text-[#2f3f92] px-6 py-3 rounded-full font-semibold transition-all duration-300 hover:bg-gray-200 flex items-center gap-2">
                        <span className="material-symbols-outlined text-sm">download</span>
                        Download Invoice
                    </button>
                    <button className="bg-gradient-to-br from-[#2b3896] to-[#4551af] text-white px-8 py-3 rounded-full font-semibold shadow-[0px_12px_32px_rgba(43,56,150,0.15)] hover:scale-95 transition-transform duration-200">
                        Contact Support
                    </button>
                </div>
            </div>
                ...
                <div className="bg-white p-8 md:p-12 rounded-xl shadow-[0px_12px_32px_rgba(43,56,150,0.06)] mb-8">
                    <div className="relative flex items-center justify-between w-full">
                        ...
                        <div className="relative z-10 flex flex-col items-center gap-3">
                            ...
                            <span className={`text-sm font-bold ${currentStep >= 1 ? 'text-[#2b3896]' : 'text-gray-400'}`}>Placed</span>
                        </div>
                        ...
                        <div className="relative z-10 flex flex-col items-center gap-3">
                            ...
                            <span className={`text-sm font-bold ${currentStep >= 2 ? 'text-[#2b3896]' : 'text-gray-400'}`}>Confirmed</span>
                        </div>
                        ...
                        <div className="relative z-10 flex flex-col items-center gap-3">
                            ...
                            <span className={`text-sm font-bold ${currentStep >= 3 ? 'text-[#2b3896]' : 'text-gray-400'}`}>Shipped</span>
                        </div>
                        ...
                        <div className="relative z-10 flex flex-col items-center gap-3">
                            ...
                            <span className={`text-sm font-medium ${currentStep === 4 ? 'text-[#2b3896] font-bold' : 'text-gray-500'}`}>Delivered</span>
                        </div>
                    </div>
                </div>
            ) : (
                <div className="bg-red-50 p-6 rounded-xl border border-red-100 mb-8 flex items-center gap-4 text-red-700">
                    <span className="material-symbols-outlined text-3xl">cancel</span>
                    <div>
                        <h3 className="font-bold text-lg">Đơn hàng đã bị hủy</h3>
                        <p className="text-sm">Đơn hàng này không còn hiệu lực. Vui lòng liên hệ bộ phận hỗ trợ nếu bạn cần thêm thông tin.</p>
                    </div>
                </div>
            )}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {/* Shipping Address */}
                        <div className="bg-gray-50/50 border border-gray-100 p-8 rounded-xl">
                            ...
                            <div className="text-gray-600 space-y-1 leading-relaxed text-sm">
                                ...
                                {order.note && <p className="mt-3 italic text-gray-500">Note: {order.note}</p>}
                            </div>
                        </div>
                        ...
                        <div className="bg-gray-50/50 border border-gray-100 p-8 rounded-xl">
                            <h3 className="text-lg font-bold mb-4 text-gray-900 flex items-center gap-2 font-['Be_Vietnam_Pro']">
                                <span className="material-symbols-outlined text-[#2b3896]">payments</span>
                                Payment Information
                            </h3>
                            <div className="space-y-5">
                                <div>
                                    <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-2">Method</p>
                                    <div className="flex items-center gap-3">
                                        ...
                                        <p className="font-medium text-gray-700 text-sm">
                                            {order.paymentMethod === 'VNPAY' ? 'Digital Wallet Transfer' : 'Cash on Delivery'}
                                        </p>
                                    </div>
                                </div>
                                <div>
                    ...
                    <div className="bg-white rounded-xl overflow-hidden shadow-[0px_12px_32px_rgba(43,56,150,0.04)] border border-gray-100">
                        <div className="px-8 py-6 border-b border-gray-100">
                            <h3 className="text-xl font-bold text-gray-900 font-['Be_Vietnam_Pro']">Order Items</h3>
                        </div>
                ...
                <div className="space-y-6">
                    <div className="bg-white p-8 rounded-xl shadow-[0px_12px_32px_rgba(43,56,150,0.06)] border border-gray-100 sticky top-28">
                        <h3 className="text-xl font-bold mb-8 text-gray-900 border-b border-gray-100 pb-4 font-['Be_Vietnam_Pro']">Order Summary</h3>
                        ...
                        <div className="space-y-4 mb-8 text-sm">
                            ...
                            <div className="flex justify-between items-center text-gray-600">
                                ...
                                <span className="font-semibold text-orange-600">- 0 <span className="text-[10px] opacity-70">₫</span></span>
                            </div>
                        </div>
                        ...
                        <div className="space-y-4">
                            <div className="bg-gray-50 border border-gray-100 p-4 rounded-xl flex gap-3">
                                ...
                                <p className="text-xs text-gray-600 leading-relaxed">
                                    Đơn hàng đang được xử lý nội bộ. Vui lòng theo dõi tiến trình tại mục trạng thái phía trên.
                                </p>
                            </div>
                            <button className="w-full bg-white text-gray-800 py-3 rounded-full font-bold border border-gray-200 hover:bg-gray-50 active:scale-95 transition-all duration-300 shadow-sm">
                    ...
                    {/* Small helper card */}
                    <div className="bg-[#2b3896] p-8 rounded-xl text-white shadow-xl shadow-indigo-900/20 relative overflow-hidden">
                        <div className="relative z-10">
                            <h4 className="text-lg font-bold mb-2 font-['Be_Vietnam_Pro']">Cần hỗ trợ?</h4>
                            <p className="text-indigo-200 text-sm mb-6 leading-relaxed">
                                Đội ngũ chăm sóc khách hàng của chúng tôi luôn túc trực 24/7 để giải đáp thắc mắc về đơn hàng của bạn.
                            </p>
                            <a href="#" className="text-white font-bold text-sm flex items-center gap-2 hover:translate-x-1 transition-transform w-max">
                                Liên hệ Trung tâm Hỗ trợ
                                <span className="material-symbols-outlined text-sm">arrow_forward</span>
                            </a>
                        </div>
                        <span className="material-symbols-outlined text-9xl text-white opacity-5 absolute -bottom-6 -right-6 pointer-events-none" style={{ fontVariationSettings: "'FILL' 1" }}>support_agent</span>
                    </div>
                </div>
            </div>
