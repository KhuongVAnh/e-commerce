...
const Checkout = () => {
  ...
  useEffect(() => {
    const loadCheckoutData = async () => {
      try {
        ...
        if (!shopIdParam) {
          toast.error(t("Không tìm thấy thông tin cần thanh toán!"));
          navigate('/cart');
          return;
        }
        ...
        if (!cartData || !cartData.shops) throw new Error(t("Giỏ hàng trống"));

        const shopData = cartData.shops.find(s => Number(s.shopId) === shopId);
        if (!shopData || shopData.items.length === 0) {
          throw new Error(t("Không có sản phẩm nào của Shop này trong giỏ hàng"));
        }
        ...
        toast.error(error.message || t("Không thể tải thông tin đơn hàng. Vui lòng thử lại!"));
        navigate('/cart');
      } finally {
        setLoading(false);
      }
    ...
    loadCheckoutData();
  }, [shopIdParam, navigate, location.state, t]);

  const handleSaveAddress = (e) => {
    e.preventDefault();
    if (!formData.fullName || !formData.phone || !formData.street || !formData.ward || !formData.city) {
      toast.error(t("Vui lòng điền đầy đủ địa chỉ giao hàng!"));
      return;
    }
    setHasSavedAddress(true);
    setShowAddressModal(false);
    toast.success(t("Đã cập nhật thông tin giao hàng!"));
  };
  ...
  const handlePlaceOrder = async () => {
    ...
    if (!hasSavedAddress) {
      toast.error(t("Vui lòng cập nhật địa chỉ giao hàng trước khi đặt hàng!"));
      return;
    }
    ...
    try {
      ...
      if (paymentMethod === 'VNPAY') {
        ...
        if (vnpayUrl) {
          ...
          toast.error(t("Lỗi: Backend không trả về link VNPay. Đơn hàng đã được lưu dưới dạng Chờ thanh toán."));
          navigate('/orders');
        }
      } else {
        toast.success(t("Đặt hàng thành công!"));
        navigate('/orders');
      }

    } catch (error) {
      console.error("Chi tiết lỗi đặt hàng:", error);
      toast.error(error.message || t("Có lỗi xảy ra khi tạo đơn!"));
    } finally {
  ...
  if (loading) {
    return (
      <div className="pt-32 pb-24 text-center min-h-screen">
        ...
        <p className="mt-4 text-gray-500 font-medium">{t('Đang chuẩn bị đơn hàng...')}</p>
      </div>
    );
  }
  ...
  return (
    <main className="pt-8 pb-24 px-6 md:px-12 max-w-screen-2xl mx-auto font-['Inter'] relative">
      ...
      <nav aria-label="Breadcrumb" className="flex items-center space-x-2 text-gray-500 text-sm mb-8 uppercase tracking-widest font-bold">
        ...
        <Link to="/cart" className="hover:text-[#2b3896] transition-colors">{t('Giỏ hàng')}</Link>
        <span className="material-symbols-outlined text-sm">chevron_right</span>
        <span className="text-[#2b3896]">{t('Thanh toán')}</span>
      </nav>

      <header className="mb-12">
        <h1 className="text-4xl md:text-5xl font-black tracking-tighter text-[#2b3896] mb-2 font-['Be_Vietnam_Pro']">{t('Thanh Toán')}</h1>
        <p className="text-gray-500 font-medium">{t('Hoàn tất các bước cuối cùng để sở hữu những sản phẩm tuyệt vời của bạn.')}</p>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <section className="lg:col-span-4 space-y-6 bg-gray-50 p-8 rounded-3xl border border-gray-100">
          <div className="flex items-center space-x-3 mb-4">
            ...
            <h2 className="text-xl font-extrabold tracking-tight text-gray-900">{t('Địa Chỉ Nhận Hàng')}</h2>
          </div>
          ...
          <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm flex flex-col items-start">
            {hasSavedAddress ? (
              ...
              <div className="py-4 w-full text-center">
                ...
                <p className="text-gray-500 text-sm italic">{t('Bạn chưa cập nhật thông tin giao hàng.')}</p>
              </div>
            )}
            ...
            >
              ...
              {hasSavedAddress ? t('Thay đổi địa chỉ') : t('Thêm địa chỉ giao hàng')}
            </button>
          </div>
        </section>
        ...
        <section className="lg:col-span-4 space-y-6">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center space-x-3">
              ...
              <h2 className="text-xl font-extrabold tracking-tight text-gray-900">{t('Tóm Tắt Đơn Hàng')}</h2>
            </div>
          </div>
          ...
          <div className="bg-white rounded-3xl p-6 shadow-[0_8px_30px_rgba(43,56,150,0.04)] border border-gray-100">
            <div className="mb-6 pb-4 border-b border-gray-100 flex items-center gap-3">
              ...
              <div>
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-0.5">{t('Giao hàng từ Shop')}</p>
              ...
              {previewData.items.map((item, index) => (
                <div key={index} className="flex gap-4 items-center">
                  ...
                  <div className="flex-grow">
                    ...
                    <p className="text-xs font-medium text-gray-500 mb-3">{t('Đơn giá:')} {Number(item.unitPrice).toLocaleString('vi-VN')}₫</p>
                    <div className="flex justify-between items-center">
                      <span className="text-xs font-bold text-gray-500 bg-gray-100 px-2 py-1 rounded-md">{t('SL:')} {item.quantity}</span>
          ...
          <div className="bg-gradient-to-br from-[#2b3896] to-[#4551af] text-white rounded-3xl p-8 shadow-2xl relative overflow-hidden">
            <div className="relative z-10">
              <div className="flex items-center space-x-3 mb-8">
                ...
                <h2 className="text-xl font-extrabold tracking-tight">{t('Thanh Toán')}</h2>
              </div>
              ...
              <div className="space-y-4 mb-10">
                <label className={`flex items-center p-4 rounded-2xl border cursor-pointer transition-colors ${paymentMethod === 'COD' ? 'bg-white/10 border-white' : 'border-white/20 hover:bg-white/5'}`}>
                  ...
                  <div className="ml-4">
                    <span className="block font-bold">{t('Khi nhận hàng (COD)')}</span>
                    <span className="text-xs text-white/70 font-medium">{t('Thanh toán bằng tiền mặt khi giao tới.')}</span>
                  </div>
                </label>
                ...
                <label className={`flex items-center p-4 rounded-2xl border cursor-pointer transition-colors ${paymentMethod === 'VNPAY' ? 'bg-white/10 border-white' : 'border-white/20 hover:bg-white/5'}`}>
                  ...
                  <div className="ml-4">
                    <span className="block font-bold">{t('Thanh toán Online (VNPay)')}</span>
                    <span className="text-xs text-white/70 font-medium">{t('Bảo mật, nhanh qua cổng VNPay.')}</span>
                  </div>
                </label>
              </div>
              ...
              <div className="space-y-3 mb-8 border-t border-white/20 pt-8 font-medium">
                ...
                <div className="flex justify-between text-sm opacity-90 items-center">
                  ...
                  <span className="text-green-300 font-bold">{previewData.pricing.shippingFee === 0 ? t('Miễn phí') : `${Number(previewData.pricing.shippingFee).toLocaleString('vi-VN')} ₫`}</span>
                </div>
              </div>
              
              <div className="flex justify-between items-end mb-10">
                <span className="text-[10px] font-bold uppercase tracking-widest opacity-80">{t('Tổng Thanh Toán')}</span>
              ...
              >
                <span>{submitting ? t('Đang xử lý...') : (paymentMethod === 'VNPAY' ? t('Thanh Toán Qua VNPay') : t('Hoàn Tất Đặt Hàng'))}</span>
          ...
          <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-lg font-extrabold text-gray-900">{t('Thông tin giao hàng')}</h3>
              </div>
            ...
            <form onSubmit={handleSaveAddress} className="p-6 space-y-5">
              <div className="group">
                <label className="block text-[11px] font-bold uppercase tracking-widest text-gray-500 mb-2 ml-1">{t('Họ và Tên')}</label>
                <input name="fullName" value={formData.fullName} onChange={handleInputChange} required className="w-full bg-gray-50 border border-gray-200 focus:border-[#2b3896] focus:ring-2 focus:ring-[#2b3896]/20 rounded-xl p-3.5 transition-all outline-none font-medium text-sm" placeholder={t("VD: Nguyễn Văn A")} type="text" />
              </div>
              <div className="group">
                <label className="block text-[11px] font-bold uppercase tracking-widest text-gray-500 mb-2 ml-1">{t('Số Điện Thoại')}</label>
                <input name="phone" value={formData.phone} onChange={handleInputChange} required className="w-full bg-gray-50 border border-gray-200 focus:border-[#2b3896] focus:ring-2 focus:ring-[#2b3896]/20 rounded-xl p-3.5 transition-all outline-none font-medium text-sm" placeholder={t("VD: 0912 345 678")} type="tel" />
              </div>
              <div className="group">
                <label className="block text-[11px] font-bold uppercase tracking-widest text-gray-500 mb-2 ml-1">{t('Số nhà / Tên đường')}</label>
                <input name="street" value={formData.street} onChange={handleInputChange} required className="w-full bg-gray-50 border border-gray-200 focus:border-[#2b3896] focus:ring-2 focus:ring-[#2b3896]/20 rounded-xl p-3.5 transition-all outline-none font-medium text-sm" placeholder={t("VD: Số 1 Đại Cồ Việt")} type="text" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="group">
                  <label className="block text-[11px] font-bold uppercase tracking-widest text-gray-500 mb-2 ml-1">{t('Phường / Xã')}</label>
                  <input name="ward" value={formData.ward} onChange={handleInputChange} required className="w-full bg-gray-50 border border-gray-200 focus:border-[#2b3896] focus:ring-2 focus:ring-[#2b3896]/20 rounded-xl p-3.5 transition-all outline-none font-medium text-sm" placeholder={t("VD: Kim Liên")} type="text" />
                </div>
                <div className="group">
                  <label className="block text-[11px] font-bold uppercase tracking-widest text-gray-500 mb-2 ml-1">{t('Tỉnh / Thành Phố')}</label>
                  <input name="city" value={formData.city} onChange={handleInputChange} required className="w-full bg-gray-50 border border-gray-200 focus:border-[#2b3896] focus:ring-2 focus:ring-[#2b3896]/20 rounded-xl p-3.5 transition-all outline-none font-medium text-sm" placeholder={t("VD: Hà Nội")} type="text" />
                </div>
              </div>
