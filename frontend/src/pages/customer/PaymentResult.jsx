  ...
  if (status === 'loading') {
    return (
      <div className="min-h-screen bg-[#f9f9fc] flex flex-col items-center justify-center px-4">
        ...
        <h2 className="text-2xl font-bold text-gray-900 mb-2 font-['Be_Vietnam_Pro']">{t('Đang xử lý thanh toán')}</h2>
        <p className="text-gray-500 text-center max-w-md">{t('Vui lòng không đóng trình duyệt. Hệ thống đang đồng bộ kết quả giao dịch từ ngân hàng một cách bảo mật...')}</p>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-[#f9f9fc] flex items-center justify-center px-4 py-24">
        <div className="bg-white p-10 md:p-14 rounded-3xl shadow-[0px_12px_32px_rgba(43,56,150,0.06)] max-w-lg w-full text-center border border-gray-100">
          ...
          <h1 className="text-3xl font-black text-gray-900 mb-4 font-['Be_Vietnam_Pro']">{t('Thanh toán thành công!')}</h1>
          <p className="text-gray-600 mb-8 leading-relaxed">
            {t('Tuyệt vời! Giao dịch của bạn đã được xác nhận.')} {orderCode && <span>{t('Mã đơn hàng của bạn là')} <strong className="text-gray-900">#{orderCode}</strong>.</span>}
          </p>
    ...
    return (
      <div className="min-h-screen bg-[#f9f9fc] flex items-center justify-center px-4 py-24">
        <div className="bg-white p-10 md:p-14 rounded-3xl shadow-[0px_12px_32px_rgba(43,56,150,0.06)] max-w-lg w-full text-center border border-gray-100">
          ...
          <h1 className="text-3xl font-black text-gray-900 mb-4 font-['Be_Vietnam_Pro']">{t('Đang xác nhận thanh toán')}</h1>
          <p className="text-gray-600 mb-8 leading-relaxed">
            {t('Giao dịch đã được gửi về hệ thống nhưng ngân hàng/VNPay có thể cần thêm thời gian để đồng bộ.')} {t('Đơn hàng')} {orderCode ? `#${orderCode}` : ''} {t('vẫn đang chờ xác nhận thanh toán.')}
          </p>
          <div className="flex flex-col gap-4">
            <Link to="/orders" className="w-full bg-[#2b3896] text-white font-bold py-4 rounded-xl hover:bg-[#1f2970] active:scale-95 transition-all shadow-md">
              {t('Kiểm tra đơn hàng')}
  ...
  return (
    <div className="min-h-screen bg-[#f9f9fc] flex items-center justify-center px-4 py-24">
      <div className="bg-white p-10 md:p-14 rounded-3xl shadow-[0px_12px_32px_rgba(43,56,150,0.06)] max-w-lg w-full text-center border border-gray-100">
        ...
        <h1 className="text-3xl font-black text-gray-900 mb-4 font-['Be_Vietnam_Pro']">{t('Thanh toán thất bại')}</h1>
        <p className="text-gray-600 mb-8 leading-relaxed">
          {t('Rất tiếc, giao dịch của bạn đã bị hủy hoặc xảy ra lỗi từ phía ngân hàng.')} {t('Đơn hàng')} {orderCode ? `#${orderCode}` : ''} {t('hiện đang ở trạng thái chờ thanh toán.')}
        </p>
        <div className="flex flex-col gap-4">
          <Link to="/orders" className="w-full bg-[#2b3896] text-white font-bold py-4 rounded-xl hover:bg-[#1f2970] active:scale-95 transition-all shadow-md">
            {t('Kiểm tra đơn hàng')}
