...
const SellerDashboard = () => {
    ...
    const getDashboardData = async () => {
      ...
      try {
        ...
        try {
          ...
          console.log(t("Seller chưa tạo gian hàng"));
        }
        ...
        console.error(t("Lỗi lấy dữ liệu dashboard seller:"), err);
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      case 'DELIVERED': return <span className="px-3 py-1 bg-emerald-50 text-emerald-600 rounded-full text-[10px] font-black uppercase">{t('Completed')}</span>;
      case 'SHIPPING': return <span className="px-3 py-1 bg-blue-50 text-blue-600 rounded-full text-[10px] font-black uppercase">{t('In Transit')}</span>;
      case 'PROCESSING': 
      case 'CONFIRMED': return <span className="px-3 py-1 bg-orange-50 text-orange-600 rounded-full text-[10px] font-black uppercase">{t('Processing')}</span>;
      case 'PENDING': return <span className="px-3 py-1 bg-rose-50 text-rose-600 rounded-full text-[10px] font-black uppercase">{t('Pending')}</span>;
      case 'CANCELLED': return <span className="px-3 py-1 bg-slate-100 text-slate-600 rounded-full text-[10px] font-black uppercase">{t('Cancelled')}</span>;
      default: return <span className="px-3 py-1 bg-slate-100 text-slate-600 rounded-full text-[10px] font-black uppercase">{t(status)}</span>;
    }
  };
  ...
  return (
    <div className="p-4 md:p-8 lg:p-10 space-y-8 bg-[#f8fafc] min-h-screen">
      <header className="flex flex-col lg:flex-row justify-between lg:items-end gap-4 mt-8 md:mt-0">
        <div><h1 className="text-2xl md:text-3xl font-black text-[#2e3785] tracking-tight">{t('Dashboard')}</h1></div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        <div className="bg-white p-6 rounded-2xl md:rounded-3xl shadow-sm border border-slate-100 flex flex-col justify-center">
          <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">{t('Total Orders')}</h3>
        ...
        <div className="bg-white p-6 rounded-2xl md:rounded-3xl shadow-sm border border-slate-100 flex flex-col justify-center">
          <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">{t('Total Revenue')}</h3>
        ...
        <div className="bg-white p-6 rounded-2xl md:rounded-3xl shadow-sm border border-slate-100 flex flex-col justify-center">
          <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">{t('Total Products')}</h3>
          <div className="flex items-end gap-2">
            <span className="text-3xl md:text-4xl font-black text-slate-900">{loading ? '...' : totalProducts}</span>
            <span className="text-[10px] font-bold text-slate-400 mb-1.5">{t('Active')}</span>
          </div>
        </div>
        <div className="bg-white p-6 rounded-2xl md:rounded-3xl shadow-sm border-l-4 border-rose-500 flex flex-col justify-center">
          <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">{t('Pending Orders')}</h3>
          <div className="flex items-end gap-2">
            <span className="text-3xl md:text-4xl font-black text-rose-600">{loading ? '...' : summary.pendingOrders.toLocaleString()}</span>
            <span className="text-[10px] font-bold text-rose-500 mb-1.5">{t('Action Required')}</span>
          </div>
        </div>
      </div>

      <div className="bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-slate-100">
        <div className="mb-8"><h2 className="text-lg font-black text-[#2e3785]">{t('Sales Performance')}</h2></div>
        <div className="h-[250px] md:h-[300px] w-full">
          {loading ? (
            <div className="w-full h-full flex items-center justify-center text-slate-400 font-bold">{t('Đang phân tích dữ liệu...')}</div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={summary.revenueSeries}>
                ...
                <Tooltip contentStyle={{borderRadius: '16px', border: 'none', boxShadow: '0 10px 25px -5px rgba(0,0,0,0.1)'}} formatter={(value) => [`${value.toLocaleString()} ₫`, t('Revenue')]} />
                <Area type="monotone" dataKey="revenue" stroke="#2e3785" strokeWidth={3} fillOpacity={1} fill="url(#colorRevenue)" />
              </AreaChart>
            </ResponsiveContainer>
          ) : (
             <div className="w-full h-full flex flex-col items-center justify-center text-slate-400"><span className="material-symbols-outlined text-4xl mb-2 opacity-50">analytics</span><span className="text-sm font-bold">{t('Chưa có giao dịch')}</span></div>
          )}
        </div>
      </div>

      <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden w-full">
        <div className="p-6 md:p-8 border-b border-slate-50 flex justify-between items-center"><h2 className="text-lg font-black text-[#2e3785]">{t('Recent Orders')}</h2></div>
        <div className="overflow-x-auto">
          <table className="w-full text-left whitespace-nowrap min-w-[700px]">
            <thead className="bg-slate-50 border-b border-slate-100">
              <tr>
                <th className="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest">{t('Order ID')}</th>
                <th className="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest">{t('Date')}</th>
                <th className="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest">{t('Customer Name')}</th>
                <th className="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest">{t('Total Amount')}</th>
                <th className="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest">{t('Status')}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-50">
              {loading ? <tr><td colSpan="5" className="p-10 text-center text-slate-400 font-bold">{t('Đang tải...')}</td></tr> : summary.recentOrders.length === 0 ? <tr><td colSpan="5" className="p-10 text-center text-slate-400">{t('Chưa có đơn hàng nào')}</td></tr> : summary.recentOrders.map(o => (
                <tr key={o.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-6 py-5 text-sm font-medium text-slate-800">{o.receiverName || t('Khách hàng')}</td>
