...
const ShopForm = () => {
  ...
  useEffect(() => {
    const loadShopInfo = async () => {
      ...
      try {
        ...
        console.log(t("Seller chưa có gian hàng. Sẵn sàng tạo mới."));
      } finally {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      setStatus({ type: 'error', msg: error.response?.data?.message || t('Có lỗi xảy ra, vui lòng thử lại!') });
    } finally {
      setLoading(false);
  ...
  if (initialLoading) return <div className="p-10 text-center font-bold text-slate-500">{t('Đang kiểm tra thông tin gian hàng...')}</div>;
      ...
      <div className="flex-1 max-w-4xl mx-auto w-full p-4 md:p-8 lg:p-12 space-y-8 md:space-y-10 mt-8 md:mt-0">
        ...
        {isUpdate && shopStatus === 'PENDING' && (
          <div className="bg-orange-50 border border-orange-200 p-4 rounded-2xl flex items-start gap-4 shadow-sm">
            ...
            <div>
              <p className="text-sm text-orange-900 font-bold mb-1">{t('Gian hàng đang chờ duyệt')}</p>
              <p className="text-xs text-orange-800 font-medium">{t('Hồ sơ của bạn đang được quản trị viên xem xét. Sản phẩm của bạn sẽ chưa được hiển thị công khai cho đến khi được duyệt.')}</p>
            </div>
          </div>
        )}
        {isUpdate && shopStatus === 'INACTIVE' && (
          <div className="bg-rose-50 border border-rose-200 p-4 rounded-2xl flex items-start gap-4 shadow-sm">
            ...
            <div>
              <p className="text-sm text-rose-900 font-bold mb-1">{t('Gian hàng đã bị khóa tạm thời')}</p>
              <p className="text-xs text-rose-800 font-medium">{t('Bạn không thể bán sản phẩm hiện tại. Vui lòng liên hệ bộ phận hỗ trợ để biết thêm chi tiết.')}</p>
            </div>
          </div>
        )}
        ...
        <div className="text-center md:text-left mb-10">
          <h1 className="text-2xl md:text-4xl font-black text-[#2e3785] mb-2 tracking-tight">
            {isUpdate ? t('Store Settings') : t('Start your business journey with us.')}
          </h1>
          <p className="text-slate-500 text-sm font-medium">
            {isUpdate ? t('Cập nhật thông tin nhận diện thương hiệu của bạn.') : t('Join a community of successful sellers and grow your business today.')}
          </p>
        </div>
        ...
        <form onSubmit={handleSave} className="bg-white p-6 md:p-10 rounded-[2rem] border border-slate-100 shadow-sm space-y-10">
          <section>
            <div className="flex items-center gap-2 mb-6 border-b border-slate-50 pb-4">
              ...
              <h2 className="text-[15px] font-black text-slate-900">{t('Visual Brand')}</h2>
            </div>
            <div>
              <label className="block text-[11px] font-black uppercase tracking-widest text-slate-500 mb-3">{t('Shop Logo')}</label>
              <div className="flex flex-col sm:flex-row items-center gap-6">
                ...
                <div className="flex-1 text-center sm:text-left w-full border-2 border-dashed border-slate-200 rounded-2xl p-6 hover:bg-slate-50 transition cursor-pointer" onClick={() => logoInputRef.current.click()}>
                  ...
                  <p className="text-sm font-bold text-slate-700 mb-1">{t('Click or drag to upload new logo')}</p>
                  <p className="text-xs text-slate-400 font-medium">{t('SVG, PNG or JPG (Min. 400x400px)')}</p>
                </div>
              </div>
            </div>
          </section>
          ...
          <section>
            <div className="flex items-center gap-2 mb-6 border-b border-slate-50 pb-4">
              ...
              <h2 className="text-[15px] font-black text-slate-900">{t('Shop Identity')}</h2>
            </div>
            <div className="space-y-6">
              <div>
                <label className="block text-[11px] font-black uppercase tracking-widest text-slate-500 mb-2">{t('Shop Name')}</label>
                <input required type="text" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} placeholder={t("e.g. The Indigo Atelier")} className="w-full bg-slate-50 border border-slate-100 px-4 py-3.5 rounded-xl text-sm font-bold text-slate-900 focus:ring-2 focus:ring-[#2e3785]/20 outline-none transition" />
              </div>
              <div>
                <label className="block text-[11px] font-black uppercase tracking-widest text-slate-500 mb-2">{t('Shop Description')}</label>
                <textarea required rows="4" value={formData.description} onChange={(e) => setFormData({...formData, description: e.target.value})} placeholder={t("Tell the story of your brand and what makes your products unique...")} className="w-full bg-slate-50 border border-slate-100 px-4 py-3.5 rounded-xl text-sm font-medium text-slate-700 focus:ring-2 focus:ring-[#2e3785]/20 outline-none resize-none transition"></textarea>
              </div>
            </div>
          </section>
          ...
          <section>
            <div className="flex items-center gap-2 mb-6 border-b border-slate-50 pb-4">
              ...
              <h2 className="text-[15px] font-black text-slate-900">{t('Business Details')}</h2>
            </div>
            <div>
              <label className="block text-[11px] font-black uppercase tracking-widest text-slate-500 mb-2">{t('Full Legal Business Address')}</label>
              <textarea required rows="2" value={formData.address} onChange={(e) => setFormData({...formData, address: e.target.value})} placeholder={t("Địa chỉ đăng ký kinh doanh/kho hàng...")} className="w-full bg-slate-50 border border-slate-100 px-4 py-3.5 rounded-xl text-sm font-medium text-slate-700 focus:ring-2 focus:ring-[#2e3785]/20 outline-none resize-none transition"></textarea>
            </div>
          </section>
          ...
          <div className="pt-6">
            {!isUpdate && (
              <div className="flex items-start gap-3 mb-6">
                ...
                <p className="text-xs text-slate-500 font-medium">{t('I agree to the')} <a href="#" className="text-[#2e3785] font-bold hover:underline">{t('Seller Handbook')}</a> {t('and the privacy guidelines of the marketplace.')}</p>
              </div>
            )}
            <button type="submit" disabled={loading} className="w-full py-4 bg-[#2e3785] hover:bg-[#252d70] text-white text-sm font-black rounded-xl shadow-lg shadow-indigo-200 transition-all disabled:opacity-70 flex justify-center items-center gap-2">
              {loading ? t('Processing...') : (isUpdate ? t('Save Changes') : t('Create My Shop'))}
            </button>
          </div>
        </form>
      </div>
